...
from app.ml.predictor import run_ml_prediction
...
def estimate_holders_from_transfer_events(transfer_events, limit=10):
    ...
    holders = sorted(holders, key=lambda h: h["balance"], reverse=True)
...
def inspect_token(token_address: str):
            ...
            if not lp_holders:
                try:
                    ...
                    if lp_transfer_logs_response.get("status") == "1":
                        raw_lp_transfer_logs = lp_transfer_logs_response.get("result", [])
                        lp_transfer_events   = decode_transfer_logs(raw_lp_transfer_logs)
                        lp_holders           = estimate_holders_from_transfer_events(
                            lp_transfer_events, limit=10
                        )
                except Exception:
                    lp_holders = []
    ...
    # ── ML Prediction (appended last, fails gracefully) ───────────────────────
    response["ml_prediction"] = run_ml_prediction(response)

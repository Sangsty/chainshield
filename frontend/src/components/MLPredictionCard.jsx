...
function getDirectionColor(direction) {
  return direction === "toward_scam" ? "bg-red-400" : "bg-emerald-400";
}

function getDirectionLabel(direction) {
  return direction === "toward_scam" ? "↑ Scam Signal" : "↓ Safe Signal";
}

function getDirectionTextColor(direction) {
  return direction === "toward_scam" ? "text-red-400" : "text-emerald-400";
}
...
// ── Anomaly Block ─────────────────────────────────────────────────────────────
function AnomalyBlock({ anomaly }) {
  if (!anomaly || anomaly.status !== "success") return null;
  ...
  const verdictColor = isAnomaly ? "text-orange-300" : "text-emerald-300";
  const scoreColor   = score > 50 ? "text-orange-400" : "text-emerald-400";

  return (
    <div className={`mt-3 rounded-2xl border p-4 ${borderColor}`}>
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">
            Anomaly Detection
          </p>
          <p className={`mt-1 text-sm font-semibold ${verdictColor}`}>
            {anomaly.verdict}
          </p>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Random Forest model · SHAP explainability · Anomaly Detection
            </p>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
            <p className="text-xs text-slate-500 leading-5">
              ...
              Green bars push toward{" "}
              <span className="text-emerald-400">safe</span>, red bars push
              toward <span className="text-red-400">scam</span>. Bar width
              shows relative importance.
            </p>
          </div>

          {/* Anomaly Detection */}
          <AnomalyBlock anomaly={mlPrediction.anomaly_detection} />
        </div>
      </div>
    </section>
  );
